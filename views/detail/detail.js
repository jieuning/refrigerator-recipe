...
const detailFetchData = async () => {
    ...
    const basicUrl = new URL(`http://211.237.50.150:7080/openapi/${API_KEY}/json/Grid_20150827000000000226_1?RECIPE_ID=${pageParams}`);
    const recipeUrl = new URL(`http://211.237.50.150:7080/openapi/${API_KEY}/json/Grid_20150827000000000228_1?RECIPE_ID=${pageParams}`);
    const ingredUrl = new URL(`http://211.237.50.150:7080/openapi/${API_KEY}/json/Grid_20150827000000000227_1?RECIPE_ID=${pageParams}`);
    ...
    try {
        ...
        const [basicResult, recipeResult, ingredResult] = await Promise.all(allRes.map(res => res.json()));
    ...
    detailContainer.innerHTML = `
        ...
        <section class="ingredient detail_section">
            <div class="main_ingred ingredient_wrap">
                ${ingredData.some(data => data.IRDNT_TY_NM === '주재료') ? '<h2 class="title_txt">주재료</h2>' : ''}
                <dl></dl>
            </div>
            <div class="sub_ingred ingredient_wrap">
                ${ingredData.some(data => data.IRDNT_TY_NM === '부재료') ? '<h2 class="title_txt">부재료</h2>' : ''}
                <dl></dl>
            </div>
            <div class="source_ingred ingredient_wrap">
                ${ingredData.some(data => data.IRDNT_TY_NM === '양념') ? '<h2 class="title_txt">양념</h2>' : ''}
                <dl></dl>
            </div>
        </section>
